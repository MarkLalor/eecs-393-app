...
# [END datastore_example_test]
    ...
    def testFilterByCourseID(self):
        ...
        query = TestModel.query(ancestor=root.key).filter(
            TestModel.courseID == 17)
        results = query.fetch(2)
        self.assertEqual(1, len(results))
        self.assertEqual(17, results[0].courseID)
    # [END datastore_example_filter]

    def testFilterByName(self):
        root = TestEntityGroupRoot(id="root")
        TestModel(parent=root.key).put()
        TestModel(name='August', parent=root.key).put()
        query = TestModel.query(ancestor=root.key).filter(
            TestModel.name == 'August')
        results = query.fetch(2)
        self.assertEqual(1, len(results))
        self.assertEqual('August', results[0].name)

    def testFilterByDepartment(self):
        root = TestEntityGroupRoot(id="root")
        TestModel(parent=root.key).put()
        TestModel(department=123, parent=root.key).put()
        query = TestModel.query(ancestor=root.key).filter(
            TestModel.department == 123)
        results = query.fetch(2)
        self.assertEqual(1, len(results))
        self.assertEqual(123, results[0].department)

    # [START datastore_example_memcache]
    def testGetEntityViaMemcache(self):
        entity_key = TestModel(courseID=18).put().urlsafe()
        retrieved_entity = GetEntityViaMemcache(entity_key)
        self.assertNotEqual(None, retrieved_entity)
        self.assertEqual(18, retrieved_entity.courseID)
