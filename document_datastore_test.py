...
# [END datastore_example_test]
    ...
    def testFilterByUser(self):
        ...
        TestModel(user='dxb448', parent=root.key).put()
        query = TestModel.query(ancestor=root.key).filter(
            TestModel.user == 'dxb448')
        results = query.fetch(2)
        self.assertEqual(1, len(results))
        self.assertEqual('dxb448', results[0].user)

    # [START datastore_example_filter]
    def testFilterByBlobKey(self):
        root = TestEntityGroupRoot(id="root")
        blobkey = blobstore.BlobKey('AMIfv95YT6H488PD6GhOL7xVrqg_juiH3Dh63yrf')
        TestModel(parent=root.key).put()
        TestModel(blob_key=blobkey, parent=root.key).put()
        query = TestModel.query(ancestor=root.key).filter(
