...
class TestModel(ndb.Model):
    ...
    courseID = ndb.IntegerProperty() # Unique ID used only by the application for mapping.

    name = ndb.StringProperty() #Human-readable course name.

    department = ndb.StringProperty()   # e.g. EECS
    number = ndb.IntegerProperty() # e.g. 393
    section = ndb.IntegerProperty() # e.g. 100

    year = ndb.IntegerProperty() # e.g. 2017
    term = ndb.StringProperty() # Spring, Summer, Fall, etc.
    '''
...
# [END datastore_example_test]
    ...
    def testGetEntityViaMemcache(self):
        entity_key = TestModel(courseID=17).put().urlsafe()
        retrieved_entity = GetEntityViaMemcache(entity_key)
        self.assertNotEqual(None, retrieved_entity)
        self.assertEqual(17, retrieved_entity.courseID)
